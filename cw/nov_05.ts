...
export function basicMath(): void {
    try {
      let i = 0;
      i = i + 300 - 200 * 4;
      let sum = 0;
  ...
  // 2️. Closure example
  export function closureExample(): void {
    try {
  ...
  export function symbolPrimitiveExample(): void {
    try {
      const js_obj = {
        name: "shankara",
        age: 60,
  ...
  // 6️. For-of loop
  export function forOfLoop(): void {
    try {
  ...
  export function classExample(): void {
    try {
      class Jedi {
        forceIsDark: boolean;
        constructor() {
  ...
  export function generatorExample(): void {
    try {
  ...
  // 10. Sync sum example
  export function sumExample(): void {
    try {
  ...
  export function logArrayElements(): void {
    try {
      const log = (el: number | undefined, i: number): void =>
        console.log(`a[${i}] = ${el}`);
      [2, 3, , 4].forEach(log);
    } catch (e) {
  ...
  export function entriesExample(): void {
    try {
  ...
  // 1️3️ Map + JSON
  export function mapJsonExample(): void {
    try {
  ...
  export function flattenExample(): void {
    try {
      function* flatten(arr: (number | number[])[]): Generator<number> {
        for (const x of arr) {
  ...
  // 1️6️ Default values
  export function defaultValuesExample(): void {
    try {
  ...
  export function reverseExample(): void {
    try {
      const reverse = ([x, ...y]: any[]): any[] => (y.length > 0 ? [...reverse(y), x] : [x]);
  ...
  // 1️8️ Simple square
  export function squaresExample(): void {
    try {
